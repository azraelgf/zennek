(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".reasons__swiper")) resizableSwiper("(max-width: 991.98px)", ".reasons__swiper", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
        ...
        if (document.querySelector(".hero__body")) {
            ...
            new Swiper(".hero__body", {
                ...
                autoplay: {
                    delay: DELAY,
                    disableOnInteraction: false,
                    pauseOnMouseEnter: !isTouch
                },
                pagination: {
                    el: ".swiper-pagination",
                    clickable: true,
                    renderBullet: (index, className) => `<span class="${className}"><i class="progress"></i></span>`
                },
                on: {
                    init(swiper) {
                        setupFormFocusPause(swiper);
                        if (isTouch) setupTouchHoldPause(swiper);
                        swiper.update();
                    }
                }
            });
            function setupFormFocusPause(swiper) {
                const form = swiper.el.querySelector("form");
                if (!form) return;
                const focusIn = e => {
                    if (form.contains(e.target)) swiper.autoplay.stop();
                };
                const focusOut = e => {
                    setTimeout(() => {
                        const active = document.activeElement;
                        const stillInside = active && form.contains(active);
                        if (!stillInside) swiper.autoplay.start();
                    }, 0);
                };
                form.addEventListener("focusin", focusIn);
                form.addEventListener("focusout", focusOut);
                form.addEventListener("submit", () => {
                    swiper.autoplay.start();
                });
            }
            function setupTouchHoldPause(swiper) {
                const el = swiper.el;
                ...
                const onEnd = () => {
                    ...
                    touchActive = false;
                    const active = document.activeElement;
                    const isInputFocused = active && (active.matches("input, textarea, select") || active.isContentEditable);
                    if (!isInputFocused) swiper.autoplay.start();
                };
        ...
        if (document.querySelector(".product-detail__images")) {
        ...
        if (document.querySelector(".swiper_mobile")) resizableSwiper("(max-width: 479.98px)", ".swiper_mobile", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
        ...
        if (document.querySelector(".swiper_other")) resizableSwiper("(max-width: 1199.98px)", ".swiper_other", {
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            spaceBetween: 20,
